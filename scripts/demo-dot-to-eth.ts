...
/**
 * Demo Script: DOT → ETH Cross-Chain Swap
 *
 * This script demonstrates a complete DOT to ETH cross-chain swap using:
...
const CONFIG = {
  ...
  // Demo parameters
  DOT_AMOUNT: "10", // 10 DOT
  DOT_SENDER: "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY", // Example DOT address
  ETH_RECIPIENT: "", // Will be derived from private key
};
...
class DOTToETHDemo {
  ...
  constructor() {
    ...
    CONFIG.ETH_RECIPIENT = this.wallet.address;
  ...
  /**
   * Run the complete DOT → ETH demo
   */
  async runDemo(): Promise<DemoResult> {
    this.startTime = Date.now();
    console.log("🚀 Starting DOT → ETH Cross-Chain Swap Demo");
  ...
  private async checkBalances(): Promise<void> {
    ...
    // Note: In a real implementation, you'd check DOT balance on the sender side
    console.log(`💰 DOT Amount to swap: ${CONFIG.DOT_AMOUNT} DOT`);
    console.log(`📍 DOT Sender: ${CONFIG.DOT_SENDER}`);
    console.log(`📍 ETH Recipient: ${CONFIG.ETH_RECIPIENT}`);
  ...
  private async createSwap() {
    ...
    const swap = await this.relayer.createDotToEthSwap(
      CONFIG.DOT_AMOUNT,
      CONFIG.DOT_SENDER,
      CONFIG.ETH_RECIPIENT
    );
  ...
  private async verifyResults(swap: any): Promise<void> {
    ...
    console.log(
      `   Secret revealed: ${swap.dotEscrow.preimage ? "✅ Yes" : "❌ No"}`
    );
  ...
  private setupEventListeners(): void {
    ...
    this.relayer.on("polkadot-contract-event", (event) => {
      console.log(`📡 Polkadot Contract Event:`, event);
    });
...
async function main(): Promise<void> {
  console.log("🌟 DOT → ETH Cross-Chain Swap Demo");
  console.log(
    "This demo will execute a complete cross-chain swap from Polkadot to Ethereum"
  );
  console.log("");
